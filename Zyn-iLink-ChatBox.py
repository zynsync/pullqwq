...
class WeChatiLinkBot:
    ...
    def _upload_media_for_account(self, account: BotAccount, file_bytes: bytes, filename: str, media_type: int, to_user_id: str) -> Optional[dict]:
        try:
            ...
            upload_timeout = max(120, int(len(encrypted) / 1024 / 1024) * 3)
...
<div class="container">
    ...
    const _renderSendingMsg = function(e) {
        ...
        o.innerHTML = '<div class="bubble out">' + bubbleContent + '<div class="msg-upload-progress"><div class="msg-upload-progress-bar"><div class="msg-upload-progress-fill"></div></div><span class="msg-upload-progress-text">0%</span></div><div class="msg-time-row"><span class="msg-send-status msg-send-loading"></span></div></div>';
    ...
    const _updateSendProgress = function(sendingId, percent, done) {
        const el = sendingId ? document.querySelector('[data-sending-id="' + sendingId + '"]') : null;
        if (!el) return;
        const box = el.querySelector('.msg-upload-progress');
        if (!box) return;
        if (done) {
        ...
        const fill = box.querySelector('.msg-upload-progress-fill');
        const txt = box.querySelector('.msg-upload-progress-text');
        if (fill) fill.style.width = p + '%';
        if (txt) txt.textContent = p + '%';
    };
    
    const _uploadMediaFile = function(mediaType, file, thumbnailData, sendingId) {
        return new Promise(function(resolve, reject) {
            ...
            o.timeout = 0;
            _updateSendProgress(sendingId, 0);
            o.upload.onprogress = function(ev) {
                if (ev.lengthComputable && ev.total > 0) {
                    _updateSendProgress(sendingId, ev.loaded * 100 / ev.total);
                }
            };
            o.upload.onload = function() {
                _updateSendProgress(sendingId, 100, true);
            };
            o.onload = function() {
            ...
            o.ontimeout = function() { reject(new Error("请求超时")); };
            o.send(file);
        });
    };
    ...
    const _sendMediaFile = async function(file, mediaType) {
        ...
        try {
            ...
            var result = await _uploadMediaFile(mediaType, file, thumbnailData, placeholderMsg.id);
...
.msg-upload-progress { display: flex; align-items: center; gap: 8px; margin-top: 6px; }
.msg-upload-progress.upload-done { display: none; }
.msg-upload-progress-bar { flex: 1; height: 4px; border-radius: 2px; background: rgba(255,255,255,0.25); overflow: hidden; }
.msg-upload-progress-fill { height: 100%; width: 0%; border-radius: 2px; background: #FFFFFF; transition: width 0.15s linear; }
.msg-upload-progress-text { font-size: 11px; color: rgba(255,255,255,0.75); min-width: 28px; text-align: right; }
